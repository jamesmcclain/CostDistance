...
import geotrellis.raster.histogram.StreamingHistogram.Bucket
...
object HistogramMixer {
  ...
  def apply(histograms: Array[StreamingHistogram], weights: Array[Double]): StreamingHistogram = {
    if ((math.max(histograms.length, weights.length) == 1) && weights.head >= 0.0)
      histograms.head
    else {
      ...
      distributions.synchronized {
        if ((math.max(histograms.length, weights.length) == 1) && weights.head < 0.0) {
          val oldHistogram = histograms.head
          val newHistogram = StreamingHistogram(oldHistogram.maxBucketCount)
          val newBuckets = oldHistogram.buckets.map({case Bucket(label, count) => Bucket(-label, count) })

          newHistogram.countItems(newBuckets)
          distributions += (key -> newHistogram)
          newHistogram
        } else {
              ...
              val distribution =
                histograms.zip(weights)
                  .reduce({ (left, right) =>
                    val (x, alpha) = left
                    val (y, beta) = right
                    (convolve(alpha, x, beta, y), math.abs(alpha) + math.abs(beta)) })._1
